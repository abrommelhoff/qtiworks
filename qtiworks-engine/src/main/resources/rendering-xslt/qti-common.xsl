  ...
  <xsl:template match="qw:itemSessionState" mode="item-status">
    <xsl:choose>
   	  <xsl:when test="exists($advanceTestItemAllowed)">
          ...
          <xsl:when test="(exists(qw:uncommittedResponseValue) or @responded='true') and @adaptive='true' and @completionStatus='unknown')">
            <span class="itemStatus answered">Incomplete</span>
          </xsl:when>
