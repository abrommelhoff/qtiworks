...
//	$result = mysql_query("SELECT t1.* FROM (SELECT DISTINCT candidate_responses.identifier, candidate_events.timestamp, candidate_events.test_event_type, candidate_responses.time_on_task, candidate_responses.misconception_type, candidate_responses.misconception_value, candidate_responses.response_correctness, candidate_session_outcomes.string_value, deliveries.lti_consumer_key_token, deliveries.lti_consumer_secret, candidate_responses.response_feedback, GROUP_CONCAT(candidate_string_response_items.string_data order by candidate_string_response_items.string_data), candidate_responses.xeid FROM lti_users JOIN candidate_sessions ON candidate_sessions.candidate_uid=lti_users.id JOIN candidate_events ON candidate_events.xid=candidate_sessions.xid JOIN candidate_responses ON candidate_responses.xeid=candidate_events.xeid JOIN candidate_string_response_items ON candidate_string_response_items.xrid=candidate_responses.xrid LEFT OUTER JOIN candidate_session_outcomes ON (candidate_session_outcomes.xid=candidate_sessions.xid AND candidate_session_outcomes.outcome_identifier= CONCAT('SCORE',candidate_responses.identifier)) JOIN deliveries ON deliveries.did=candidate_sessions.did WHERE lti_user_id=".$_REQUEST["id"]." GROUP BY candidate_responses.identifier, candidate_events.timestamp ) t1 WHERE t1.timestamp = ( SELECT MAX(t2.timestamp) FROM ( SELECT DISTINCT candidate_responses.identifier, candidate_events.timestamp FROM lti_users JOIN candidate_sessions ON candidate_sessions.candidate_uid=lti_users.id JOIN candidate_events ON candidate_events.xid=candidate_sessions.xid JOIN candidate_responses ON candidate_responses.xeid=candidate_events.xeid WHERE lti_user_id=".$_REQUEST["id"]." GROUP BY candidate_responses.identifier, candidate_events.timestamp) t2 WHERE t2.identifier = t1.identifier) and t1.xeid = (SELECT MAX(t3.xeid) FROM (SELECT DISTINCT candidate_responses.identifier, candidate_events.timestamp, candidate_events.xeid FROM lti_users JOIN candidate_sessions ON candidate_sessions.candidate_uid=lti_users.id JOIN candidate_events ON candidate_events.xid=candidate_sessions.xid JOIN candidate_responses ON candidate_responses.xeid=candidate_events.xeid WHERE lti_user_id=".$_REQUEST["id"]." GROUP BY candidate_responses.identifier, candidate_events.timestamp, candidate_events.xeid) t3 WHERE t3.identifier = t1.identifier)");
	$result = mysql_query("SELECT DISTINCT b.identifier, b.timestamp, b.test_event_type, b.time_on_task, b.misconception_type, b.misconception_value, b.response_correctness, b.string_value, b.lti_consumer_key_token, b.lti_consumer_secret, b.response_feedback, GROUP_CONCAT(b.string_data ORDER BY b.string_data), b.xeid, b.response_identifier from ( SELECT t1.* FROM ( SELECT DISTINCT candidate_responses.identifier, candidate_events.timestamp, candidate_events.test_event_type, candidate_responses.time_on_task, candidate_responses.misconception_type, candidate_responses.misconception_value, candidate_responses.response_correctness, candidate_session_outcomes.string_value, deliveries.lti_consumer_key_token, deliveries.lti_consumer_secret, candidate_responses.response_feedback, GROUP_CONCAT(candidate_string_response_items.string_data ORDER BY candidate_string_response_items.string_data) string_data, candidate_responses.xeid, candidate_responses.response_identifier FROM lti_users JOIN candidate_sessions ON candidate_sessions.candidate_uid=lti_users.id JOIN candidate_events ON candidate_events.xid=candidate_sessions.xid JOIN candidate_responses ON candidate_responses.xeid=candidate_events.xeid JOIN candidate_string_response_items ON candidate_string_response_items.xrid=candidate_responses.xrid LEFT OUTER JOIN candidate_session_outcomes ON (candidate_session_outcomes.xid=candidate_sessions.xid AND candidate_session_outcomes.outcome_identifier= CONCAT('SCORE',candidate_responses.identifier)) JOIN deliveries ON deliveries.did=candidate_sessions.did WHERE lti_user_id=".$_REQUEST["id"]." GROUP BY candidate_responses.identifier, candidate_events.timestamp) t1 WHERE t1.timestamp = ( SELECT MAX(t2.timestamp) FROM ( SELECT DISTINCT candidate_responses.identifier, candidate_events.timestamp, candidate_responses.response_identifier FROM lti_users JOIN candidate_sessions ON candidate_sessions.candidate_uid=lti_users.id JOIN candidate_events ON candidate_events.xid=candidate_sessions.xid JOIN candidate_responses ON candidate_responses.xeid=candidate_events.xeid WHERE lti_user_id=".$_REQUEST["id"]." GROUP BY candidate_responses.identifier, candidate_events.timestamp, candidate_responses.response_identifier) t2 WHERE t2.identifier = t1.identifier and t2.response_identifier = t1.response_identifier) AND t1.xeid = ( SELECT MAX(t3.xeid) FROM ( SELECT DISTINCT candidate_responses.identifier, candidate_events.timestamp, candidate_events.xeid, candidate_responses.response_identifier FROM lti_users JOIN candidate_sessions ON candidate_sessions.candidate_uid=lti_users.id JOIN candidate_events ON candidate_events.xid=candidate_sessions.xid JOIN candidate_responses ON candidate_responses.xeid=candidate_events.xeid WHERE lti_user_id=".$_REQUEST["id"]." GROUP BY candidate_responses.identifier, candidate_events.timestamp, candidate_events.xeid, candidate_responses.response_identifier) t3 WHERE t3.identifier = t1.identifier and t3.response_identifier = t1.response_identifier) ) b group by identifier");
